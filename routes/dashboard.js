  ...
  if (req.session.user && req.session.user.tipo_usuario === 'restaurante') {
    try {
      ...
      if (restaurants.length > 0) {
        ...
        console.log('DEBUG: pedidosNoEntregados en res.locals:', res.locals.pedidosNoEntregados);
      }
    } catch (error) {
      console.error('Error obteniendo conteo de pedidos no entregados:', error);
...
router.post('/settings', requireRestaurant, requireVerifiedRestaurant, uploadRestaurantImages.fields([{ name: 'imagen_logo', maxCount: 1 }, { name: 'imagen_banner', maxCount: 1 }]), [
  ...
  if (req.body.edit_user) {
    console.log('Processing user data update');
    return await handleUserUpdate(req, res);
  } else if (req.body.edit_transfer) {
    console.log('Processing transfer data update');
    return await handleTransferUpdate(req, res);
  } else {
    console.log('Processing restaurant settings update');
...
async function handleUserUpdate(req, res) {
  try {
    ...
    req.session.save(err => {
      ...
      console.log('User data updated successfully and session saved');
      res.json({
        success: true,
...
async function handleRestaurantSettingsUpdate(req, res) {
  ...
  try {
    ...
    console.log('Dias operacion received:', dias_operacion, typeof dias_operacion);
    ...
    if (Array.isArray(dias_operacion)) {
      ...
      console.log('Dias operacion is array:', diasOperacionArray);
    } else if (typeof dias_operacion === 'string') {
      try {
        diasOperacionArray = JSON.parse(dias_operacion);
        console.log('Dias operacion parsed from string:', diasOperacionArray);
      } catch (e) {
    ...
    if (diasOperacionArray.length === 0) {
      console.log('No dias operacion selected');
    ...
    console.log('Dias operacion JSON:', diasOperacionJSON);
    ...
    if (emailUpdated) {
      req.session.save(err => {
        ...
        console.log('Session saved after email update');
        sendResponse();
      });
    } else {
      sendResponse();
...
router.get('/products', requireRestaurant, requireVerifiedRestaurant, getPendingOrdersCount, async (req, res) => {
  try {
    ...
    console.log('DEBUG dashboard productos:', {
      sql,
      queryParams,
      filterValues,
      limit,
    ...
    const [categorias] = await db.execute(`
      SELECT id, nombre FROM categorias_productos WHERE restaurante_id = ? OR restaurante_id IS NULL ORDER BY orden_display, nombre
    `, [restaurantId]);
...
router.post('/products/add', requireRestaurant, requireVerifiedRestaurant, uploadProductImage.single('imagen'), [
  ...
  try {
    ...
    if (req.file) {
      ...
      console.log('Image path:', imagenPath);
    }
...
router.get('/drivers', requireRestaurant, requireVerifiedRestaurant, getPendingOrdersCount, async (req, res) => {
  try {
    ...
    const [drivers] = await db.execute(`
      SELECT u.id, u.nombre, u.apellido, u.email, u.telefono, d.vehicle_type, d.status
      FROM usuarios u
...
router.post('/drivers/add', requireRestaurant, requireVerifiedRestaurant, [
  ...
  console.log('Received driver add request. Body:', req.body);
  const connection = await db.getConnection();
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
    ...
    // Create driver entry
    await connection.execute(
      `INSERT INTO drivers (user_id, restaurante_id, vehicle_type)
       VALUES (?, ?, ?)`
...
router.get('/orders', requireRestaurant, requireVerifiedRestaurant, getPendingOrdersCount, async (req, res) => {
  try {
    ...
    if (search) {
      sql += ` AND (p.numero_pedido LIKE ? OR u.nombre LIKE ? OR u.apellido LIKE ?)`;
      countSql += ` AND (p.numero_pedido LIKE ? OR u.nombre LIKE ? OR u.apellido LIKE ?)`;
    ...
    // Validación y logs de depuración
    console.log('DEBUG dashboard pedidos:', {
      sql,
      queryParams,
      filterValues,
      limit,
...
router.get('/orders/:id', requireRestaurant, requireVerifiedRestaurant, getPendingOrdersCount, async (req, res) => {
  try {
    ...
    const [drivers] = await db.execute(`
      SELECT id, nombre, apellido FROM usuarios WHERE tipo_usuario = 'repartidor' AND activo = 1
    `);
...
router.post('/orders/:id/assign-driver', requireRestaurant, requireVerifiedRestaurant, async (req, res) => {
  ...
  try {
    ...
    // Check if driver exists and is a 'repartidor'
        const [driver] = await connection.execute(`SELECT id FROM usuarios WHERE id = ? AND tipo_usuario = 'repartidor'`, [driver_id]);
    if (driver.length === 0) {
...
router.post('/orders/:id/request-private-driver', requireRestaurant, requireVerifiedRestaurant, async (req, res) => {
  ...
  try {
    ...
    const [availableDrivers] = await connection.execute(
      "SELECT u.id, u.nombre, u.apellido FROM usuarios u JOIN drivers d ON u.id = d.user_id WHERE u.tipo_usuario = 'repartidor' AND d.restaurante_id IS NULL AND d.status = 'available' LIMIT 1"
    );
    ...
    const cobrosConMP = cobrosRecientes.map(cobro => {
      if (cobro.estado === 'pendiente' && mpConfigured) {
        ...
        const paymentData = {
          ...
          unit_price: parseFloat(cobro.monto_comision),
          quantity: 1,
          external_reference: `cobro_${cobro.id}`,
          notification_url: `${process.env.BASE_URL || 'http://localhost:3000'}/webhooks/mercadopago`
        };

        // For now, create a simple payment link (in production, this would use MercadoPago API)
        const mercadopago_url = `/dashboard/payments/mercadopago/create?cobro_id=${cobro.id}&amount=${cobro.monto_comision}`;
...
// Mercado Pago OAuth Redirect
router.get('/mercadopago/auth', requireRestaurant, requireVerifiedRestaurant, (req, res) => {
  const userId = req.session.user.id;
  const authUrl = `https://auth.mercadopago.com.ar/authorization?client_id=${process.env.MP_APP_ID}&response_type=code&platform_id=mp&state=${userId}&redirect_uri=${process.env.MP_REDIRECT_URI}`;
...
router.get('/mercadopago/callback', requireRestaurant, requireVerifiedRestaurant, async (req, res) => {
  ...
  if (!code) {
    return res.redirect('/dashboard/payments?error=mp_auth_failed');
  }
  ...
  try {
    ...
    await db.execute(`
      UPDATE restaurantes SET mp_access_token = ?, mp_refresh_token = ?, mp_user_id = ?
      WHERE usuario_id = ?
    `, [access_token, refresh_token, user_id, userId]);

    res.redirect('/dashboard/payments?success=mp_auth_success');
  } catch (error) {
    console.error('Error in Mercado Pago callback:', error);
    res.redirect('/dashboard/payments?error=mp_token_failed');
  }
});
...
router.get('/payments/mercadopago/create', requireRestaurant, requireVerifiedRestaurant, async (req, res) => {
  try {
    ...
    if (!cobro.mp_access_token || !cobro.mp_user_id) {
      return res.status(400).json({ success: false, message: 'MercadoPago no está configurado para este restaurante' });
    }
    ...
    const preference = {
      ...
      back_urls: {
        success: `${process.env.BASE_URL}/dashboard/payments?payment_status=success`,
        failure: `${process.env.BASE_URL}/dashboard/payments?payment_status=failure`,
        pending: `${process.env.BASE_URL}/dashboard/payments?payment_status=pending`
      },
